...
export function setEncrypted(encrypted) {
  return Cookies.set(ENCRYPTED_KEY, encrypted);
}

export function removeEncrypted() {
  return Cookies.remove(ENCRYPTED_KEY);
}
...
export async function showError(errorMsg = '', separator = '$@$') {
  for (const msg of errorMsg.split(separator)) {
    await nextTick();
